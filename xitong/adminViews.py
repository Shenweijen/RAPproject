...
#  从页面post数据中获取管理区域   
    manageareastr = request.POST.get("managearea");
    managearea = "";
    if(manageareastr is not None):
        managearea = manageareastr;

#  将管理员的属性赋值给管理员，生成管理员对象   
    admin = models.Admin(name=name,username=username,password=password,email=email,managearea=managearea,);
...
#  返回页面提示信息,添加管理员成功,并跳转到管理员管理页面   
    return HttpResponse(u"<script>alert('添加管理员成功');window.location.href = '/admin/adminguanli';</script>");
...
#  从页面post数据中获取姓名并赋值给admin的name字段   
    namestr = request.POST.get("name");
    if(namestr is not None):
        admin.name = namestr;

#  从页面post数据中获取用户名并赋值给admin的username字段   
    usernamestr = request.POST.get("username");
    if(usernamestr is not None):
...
#  从页面post数据中获取邮箱并赋值给admin的email字段   
    emailstr = request.POST.get("email");
    if(emailstr is not None):
        admin.email = emailstr;

#  从页面post数据中获取管理区域并赋值给admin的managearea字段   
    manageareastr = request.POST.get("managearea");
    if(manageareastr is not None):
        admin.managearea = manageareastr;
...
#  返回页面提示信息,修改管理员成功,并跳转到管理员管理页面   
    return HttpResponse(u"<script>alert('修改管理员成功');window.location.href = '/admin/adminguanli';</script>");
...
#  返回页面提示信息,删除管理员成功,并跳转到管理员管理页面   
    return HttpResponse(u"<script>alert('删除管理员成功');window.location.href = '/admin/adminguanli';</script>");
...
#  使用django的filter方法过滤查询包含search的管理员信息   
    adminall = models.Admin.objects.filter(name__icontains=search);
